...
namespace Park20.Backoffice.Application.Services
{
    public class PaymentService : IPaymentService
    {
        ...
        {
            var completeQueryString = "mutation($input: PaymentRequestInput!){ processPayment(paymentRequest: $input){ successful, confirmationString } }";
            try
            {
                ...
                return response.Data.processPayment.successful;
            }
            catch (Exception ex)
            {
                throw;
